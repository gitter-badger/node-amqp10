...
var EventEmitter = require('events').EventEmitter,
    ...
    States = require('./types/delivery_state'),
...
Link.prototype.messageReceived = function(transferFrame) {
  ...
  this.settleMessages(transferFrame.deliveryId);
  this.emit(Link.MessageReceived, transferFrame.message);
  this._checkCredit();
};

Link.prototype.settleMessages = function(firstDeliveryId, lastDeliveryId, state) {
  var dispoFrame = new DispositionFrame({ role: constants.linkRole.receiver, first: firstDeliveryId, settled: true });
  if (lastDeliveryId) dispoFrame.last = lastDeliveryId;
  dispoFrame.state = state || new States.Accepted();
  dispoFrame.channel = this.session.channel;
  dispoFrame.handle = this.handle;
  this.session.connection.sendFrame(dispoFrame);
};
